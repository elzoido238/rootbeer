...
ROOTLIBS  = $(shell root-config --glibs) -lXMLParser -lTreePlayer
...
CXXFLAGS  = $(DEBUG) $(OPTIMIZE) $(INCFLAGS) $(ROOTFLAGS) $(DEFAULTS)
CINTFLAGS = $(DEBUG) $(OPTIMIZE) $(INCFLAGS) $(DEFAULTS)
...
clean:
	rm -rf $(RBLIB)/*.so rootbeer $(RBLIB)/*.dSYM ./*.dSYM $(CINT)/*Dict*.h $(CINT)/*Dict*.cxx $(OBJ)/*.o $(OBJ)/*/*.o
