...
RB_ROOTMAP := $(patsubst %.so,%.rootmap,$(RBlib))

RBmid        = $(RBLIB)/librbMidas.so
MID_ROOTMAP := $(patsubst %.so,%.rootmap,$(RBmid))

ifeq ($(ROOTMAJORVERSION),6)
MAKE_RBDICT   += $(ROOTCINT) -v4 -f $@ -s $(RBlib) -rml $(RBlib) -rmf $(RB_ROOTMAP) -c $(CINTFLAGS) \
-p $(HEADERS) $(CINT)/Linkdef.h
MAKE_MID_DICT += $(ROOTCINT) -v4 -f $@ -s $(RBmid) -rml $(RBmid) -rmf $(MID_ROOTMAP) -c $(CINTFLAGS) \
-p $(MIDAS_HEADERS) $(SRC)/midas/MidasLinkdef.h
else
MAKE_RBDICT   += $(ROOTCINT) -f $@ -c $(CINTFLAGS) -p $(HEADERS) $(CINT)/Linkdef.h
MAKE_MID_DICT += $(ROOTCINT) -f $@ -c $(CINTFLAGS) -p $(MIDAS_HEADERS) $(SRC)/midas/MidasLinkdef.h
endif
